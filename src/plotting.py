...
class MPLPlotWrapper(object):
    ...
    def write_fig_to_file(self, fname, format='pdf'):
        """
...
def get_largest_factors(N : int) -> int:
    """
    Returns the largest two integer factors of an integer
    """
    # Handles the composite numbers
    a = 0; b = 0;
    c = np.floor(np.sqrt(N))
    while ((a == 0) and (b == 0) and (c < N)):
        if (N % c) == 0:
            a = c
            b = N / c
            break
